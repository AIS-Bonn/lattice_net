...
public:
    ...
    // torch::Tensor values();
    // torch::Tensor sliced_values_hom();
    // torch::Tensor lattice_rowified();
...
private:
    ...
    // void create_program_handles(); ///it uses Jittify to get a handle for the programs. The programs can contain more than one kernel.. It doesnt actually compile them, they will get jit compiled the first time you run them
    ...
    //the matrices would need to multiply from the RIGHT, because if you multiply from th eleft they will be gigantic. So therefore they would need to be transpose. For the moment I won't use them because I want only a working example. Also I have to really think about how these 3 matrices work together and which way the multiply to create the  m_embedding_matrix
    // torch::Tensor create_E_matrix(); //elevated the points from d dimensional to (d+1) where the hyperplane H_d lives
    // torch::Tensor create_scaling_matrix(); //scales each axis as to account for the variance introduced in splatting and slicing
    // torch::Tensor create_sigmas_matrix(); //scales the positions by the inverse of the sigmas defined
